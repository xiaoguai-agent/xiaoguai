...
impl TaskBoardRepository for InMemoryTaskBoardRepository {
    ...
    async fn dispatch_next_ready(
        ...
        let task = store.tasks.iter_mut().find(|t| t.id == task_id).unwrap();
...
mod tests {
    ...
    async fn list_tasks_filtered_by_column() {
        ...
        let triage = repo
            .list_tasks(board.id, Some(Column::Triage))
            .await
            .unwrap();
        let ready = repo
            .list_tasks(board.id, Some(Column::Ready))
            .await
            .unwrap();
    ...
    async fn full_lifecycle_history_ordered_by_occurred_at() {
        ...
        repo.update_task_column(task.id, Column::Todo, "u", None)
            .await
            .unwrap();
        repo.update_task_column(task.id, Column::Ready, "u", None)
            .await
            .unwrap();
        repo.dispatch_next_ready(board.id, "agent-1").await.unwrap();
        repo.update_task_column(task.id, Column::Done, "agent-1", None)
            .await
            .unwrap();
