//! Mistral La Plateforme backend.
//!
//! Endpoint: `https://api.mistral.ai/v1/chat/completions`
//! Auth: `Authorization: Bearer <key>` (same as OpenAI)
//!
//! The wire protocol is OpenAI-compatible for text streaming. Mistral tool
//! calling uses the same `tools` + `tool_calls` structure as OpenAI,
//! so we reuse the OpenAI SSE parsing path wholesale.
//!
...
//!   - `open-mistral-nemo`
//!
//! Note: Mistral's `finish_reason` values use `"stop"` and `"tool_calls"`,
//! which are identical to OpenAI's — no custom mapping required.
...
pub const MISTRAL_DEFAULT_BASE: &str = "https://api.mistral.ai";
const MISTRAL_BASE: &str = MISTRAL_DEFAULT_BASE;
...
impl MistralBackend {
    /// Production constructor. `api_key` is the `MISTRAL_API_KEY` secret.
    pub fn new(api_key: impl Into<String>) -> Self {
        Self::with_base_url(MISTRAL_BASE, api_key)
    }
...
struct MistralMessage<'a> {
    ...
    content: &'a str,
    #[serde(skip_serializing_if = "Vec::is_empty")]
    tool_calls: Vec<MistralOutgoingToolCall<'a>>,
    #[serde(skip_serializing_if = "Option::is_none")]
    tool_call_id: Option<&'a str>,
}

#[derive(Serialize)]
struct MistralOutgoingToolCall<'a> {
    id: &'a str,
    #[serde(rename = "type")]
    kind: &'static str,
    function: MistralOutgoingFn<'a>,
}

#[derive(Serialize)]
struct MistralOutgoingFn<'a> {
    name: &'a str,
    arguments: &'a str,
}
...
struct MistralTool<'a> {
    #[serde(rename = "type")]
    kind: &'static str,
    function: MistralToolFn<'a>,
}

#[derive(Serialize)]
struct MistralToolFn<'a> {
    name: &'a str,
    #[serde(skip_serializing_if = "Option::is_none")]
...
struct MistralDelta {
    ...
    #[serde(default)]
    tool_calls: Vec<MistralIncomingToolCallDelta>,
}

#[derive(Deserialize)]
struct MistralIncomingToolCallDelta {
    index: u32,
    #[serde(default)]
    id: Option<String>,
    #[serde(default)]
    function: Option<MistralIncomingFnDelta>,
}

#[derive(Deserialize)]
struct MistralIncomingFnDelta {
    #[serde(default)]
    name: Option<String>,
    #[serde(default)]
...
fn build_messages(messages: &[Message]) -> Vec<MistralMessage<'_>> {
    messages
        .iter()
        .map(|m| MistralMessage {
            ...
            tool_calls: m
                .tool_calls
                .iter()
                .map(|tc| MistralOutgoingToolCall {
                    id: &tc.id,
                    kind: "function",
                    function: MistralOutgoingFn {
                        name: &tc.name,
...
fn build_tools(tools: &[ToolSpec]) -> Vec<MistralTool<'_>> {
    tools
        .iter()
        .map(|t| MistralTool {
            kind: "function",
...
fn build_tool_choice(c: &ToolChoice) -> Option<JsonValue> {
    match c {
        ...
        ToolChoice::Required => Some(JsonValue::String("any".to_string())),
...
impl PartialToolCall {
    fn merge(&mut self, delta: MistralIncomingToolCallDelta) {
    ...
    fn into_complete(self, index: u32) -> Option<ToolCallSpec> {
        let id = self.id.unwrap_or_else(|| format!("call_mistral_{index}"));
        let name = self.name?;
        Some(ToolCallSpec {
            id,
            name,
    ...
    async fn chat_stream(&self, req: ChatRequest) -> Result<ChatStream, LlmError> {
        ...
        let mapped = {
            ...
            sse.map(move |ev| {
                ...
                let parsed: MistralSseChunk = serde_json::from_str(&ev.data)
