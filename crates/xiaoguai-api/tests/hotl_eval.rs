...
async fn eval_under_threshold_all_allow() {
    let tid = Uuid::new_v4();
    let store = count_store(tid, "llm_call", 3600, 100, Some("ops@example.com"));
    let enforcer = InMemoryHotlEnforcer::new(store);

    for i in 0..10 {
        let verdict = enforcer.check(tid, "llm_call", 1.0).await.unwrap();
        assert_eq!(
            verdict,
            HotlVerdict::Allow,
...
async fn eval_at_threshold_still_allow() {
    const N: i32 = 15;
    let tid = Uuid::new_v4();
    let store = count_store(tid, "llm_call", 3600, N, Some("ops@example.com"));
    let enforcer = InMemoryHotlEnforcer::new(store);
...
async fn eval_above_threshold_escalates() {
    ...
    let tid = Uuid::new_v4();
    let store = count_store(tid, "llm_call", 3600, N, Some(dest));
    let enforcer = InMemoryHotlEnforcer::new(store);
...
async fn eval_above_threshold_denies_when_no_escalate_to() {
    const N: i32 = 5;
    let tid = Uuid::new_v4();
    let store = count_store(tid, "llm_call", 3600, N, None);
    let enforcer = InMemoryHotlEnforcer::new(store);
...
async fn eval_window_expiry_old_events_excluded() {
    ...
    let tid = Uuid::new_v4();
    let store = count_store(tid, "llm_call", 1, N, Some("ops@example.com"));
    let enforcer = InMemoryHotlEnforcer::new(store);
...
/// Tenant A exhausting its budget must not affect tenant B.
#[tokio::test]
async fn eval_multi_tenant_isolation() {
    let tid_a = Uuid::new_v4();
    let tid_b = Uuid::new_v4();

    // Both tenants share the same store instance.
    let store = Arc::new(InMemoryHotlPolicyStore::new());

    // Tenant A: tight budget of 3, with Deny on breach.
    store.seed(HotlPolicy {
        id: Uuid::new_v4(),
        tenant_id: tid_a,
    ...
    // Tenant B: generous budget of 100.
    store.seed(HotlPolicy {
        id: Uuid::new_v4(),
        tenant_id: tid_b,
        scope: "llm_call".to_owned(),
    ...
    // Tenant A: 3 calls at-threshold (Allow), then breach (Deny).
    for _ in 0..3 {
        let v = enforcer.check(tid_a, "llm_call", 1.0).await.unwrap();
        assert_eq!(v, HotlVerdict::Allow, "tenant A within budget");
    }
    let v = enforcer.check(tid_a, "llm_call", 1.0).await.unwrap();
    assert!(
        matches!(v, HotlVerdict::Deny(_)),
        "tenant A 4th call must Deny; got {v:?}"
    );

    // Tenant B's calls must be unaffected — all Allow.
    for i in 0..20 {
        let v = enforcer.check(tid_b, "llm_call", 1.0).await.unwrap();
        assert_eq!(
            v,
            HotlVerdict::Allow,
            "tenant B call {i} must be Allow (not polluted by tenant A)"
        );
    }
}

// ── scenario 6: policy hot-update ────────────────────────────────────────────
...
async fn eval_tier_routing_low_risk() {
    let tid = Uuid::new_v4();
    let store = count_store(tid, "email_send", 3600, 1, Some("tier-1@example.com"));
    let enforcer = InMemoryHotlEnforcer::new(store);
...
async fn eval_tier_routing_medium_risk() {
    let tid = Uuid::new_v4();
    let store = count_store(tid, "llm_call", 3600, 1, Some("tier-2@example.com"));
    let enforcer = InMemoryHotlEnforcer::new(store);
...
async fn eval_tier_routing_high_risk() {
    let tid = Uuid::new_v4();
    let store = count_store(tid, "external_api", 3600, 1, Some("tier-3@example.com"));
    let enforcer = InMemoryHotlEnforcer::new(store);
...
async fn eval_tier_routing_deny_beats_escalate() {
    ...
    let v = enforcer.check(tid, "llm_call", 1.0).await.unwrap(); // breach
...
async fn eval_usd_budget_breach_escalates() {
    ...
    let store = usd_store(tid, "llm_call", 3600, 1.0, Some("billing@example.com"));
    let enforcer = InMemoryHotlEnforcer::new(store);
