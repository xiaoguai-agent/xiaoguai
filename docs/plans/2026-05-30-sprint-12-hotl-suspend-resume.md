...
| P0 | S12-3 | **`DecisionRegistry` on AppState** — new module `crates/xiaoguai-api/src/hotl/decision_registry.rs`. `pub struct DecisionRegistry { waiters: DashMap<Uuid, oneshot::Sender<HotlDecisionVerdict>> }`. Methods `register(request_id, expires_at) -> HotlSuspensionTicket` (creates oneshot pair, stores sender keyed by `request_id`, spawns a `tokio::time::sleep_until` companion that removes the entry on expiry), `resolve(request_id, verdict) -> bool` (pop sender from map, `send(verdict)`, return whether a live waiter existed). Add to `AppState` at `crates/xiaoguai-api/src/state.rs` next to `hotl_decision_store` (line 208) as `pub decision_registry: Arc<DecisionRegistry>` — always-present (no `Option`), because the registry itself has zero side-effects when no one calls `register`. Unit tests: register-then-resolve returns true; resolve-on-empty returns false; timeout removes entry; concurrent register/resolve race is safe (use `DashMap::remove`). | none | 0.75 | R |
...
| P0 | S12-5 | **ReAct loop integration** — extend `crates/xiaoguai-agent/src/react.rs` gate-match site (lines 442-456). New arm for `HotlGateVerdict::Suspend { request_id, scope, ticket }`: emit `AgentEvent::HotlPending { request_id, tool: name, args_redacted: args_json, scope, expires_at }` via the existing event channel; call `ticket.await_decision(&cancel).await`; match resolution → emit `AgentEvent::HotlResolved {...}`; on `Allow` fall through to existing dispatch path; on `Deny`/`Timeout` synthesise `ToolDispatchOutcome { ok: false, error: Some(reason) }` (mirroring the existing Deny short-circuit at line 449). On cancel during `await_decision`: do NOT emit `HotlResolved` — the cancel path is handled by the existing iteration-boundary cancel logic (`Final(Cancelled)`). **Note on arg redaction**: sprint-12 passes `args_json` through unmodified — policy-driven redaction is a sprint-13 follow-up (see §4 out-of-scope). Unit tests for the new arm: happy-path-allow, happy-path-deny, timeout-synthesises-deny, cancel-wins-over-resolve. | S12-1, S12-2 | 1.5 | R E |
| P0 | S12-6 | **`POST /v1/hotl/decisions` resolves the waiter** — extend the sprint-11 handler at `crates/xiaoguai-api/src/routes/hotl_decisions.rs` to call `state.decision_registry.resolve(request_id, HotlDecisionVerdict { verdict: ..., decided_by, recorded_at })` after persisting the decision (and after the raise_policy in-tx create if present). Set `HotlDecisionResponse.resumed` to the return value (was hardcoded `false`). The persist step still runs **before** the resolve so a registry crash doesn't lose the operator's audit trail. Extend existing integration tests in `crates/xiaoguai-api/tests/hotl_decisions.rs`: new cases `decision_resolves_live_waiter_returns_resumed_true`, `decision_with_no_waiter_returns_resumed_false`, `late_decision_after_timeout_returns_resumed_false_and_409_is_unchanged`. The 8 existing sprint-11 cases continue to pass (resumed=false everywhere except the new waiter case). | S12-3, S12-7 | 0.75 | T S |
...
| P0 | S12-8 | **Frontend SSE event handling + banner state machine** — extend `frontend/shared/src/agentEventStream.ts` to recognise the two new event kinds (`hotl_pending`, `hotl_resolved`), parse them into typed `AgentEvent` discriminated-union members, and surface via the existing `useStreamingMessage` reducer. Update `frontend/chat-ui/src/HotlBanner.tsx`: drop the 5 s optimistic-clear timeout (remove the `setTimeout` block); add `useEffect` keyed on `pending.request_id` that listens to incoming `hotl_resolved` events from the stream; on match clear the banner; on `verdict=timeout` show the `chat.hotl.timeout_annotation` label before clearing after 3 s. Handle conflict: if local `submitHotlDecision` succeeds but the eventual `hotl_resolved` carries a different `decided_by` (sibling tab won the race), revert local submitting state + show a one-line `chat.hotl.conflict_toast`. New i18n keys: `chat.hotl.{timeout_annotation, conflict_toast}` × 3 locales. Extend `HotlBanner.test.tsx` with 4 cases: optimistic-clear-removed, resolve-via-sse, timeout-annotation, sibling-tab-conflict. | S12-2 (SSE wire shape) | 1.25 | T R |
| P0 | S12-9 | **Backend integration tests** — three new files under `crates/xiaoguai-agent/tests/`: `hotl_suspend.rs` (gate returns Suspend → loop emits HotlPending → registry.resolve(Allow) → loop emits HotlResolved + dispatches tool → ToolCallFinished with ok=true); `hotl_suspend_timeout.rs` (no resolve call within configured expiry → ticket.await_decision returns Timeout → HotlResolved(Timeout) + synthetic ToolCallFinished with ok=false); `hotl_suspend_cancel.rs` (cancellation token fires during ticket.await_decision → Final(Cancelled) emitted, NO HotlResolved). Each test uses an in-mem `DecisionRegistry` (shared with a mock `HotlEnforcer` that always Escalates) and asserts the exact event sequence. | S12-4, S12-5 | 1.0 | T R |
...
**Total: ~9.6 dev-days** (10 P0 tasks + 1 P1 doc cleanup; close to the design's "~2 days" hand-wave because the design only counted the agent-side wiring, not the PG store + frontend + e2e).

---

## 2. Sub-agent dispatch plan

Four parallel work streams once Step 3 review passes. Dependency-driven phasing:

| Phase | Stream A (rust types) | Stream B (control plane) | Stream C (PG + route) | Stream D (frontend) | Sync point |
|---|---|---|---|---|---|
| Phase A (day 0) | S12-0 housekeeping merged first as a tiny PR (config flag default `false` makes the whole sprint a no-op until S12-4 + S12-5 land) | — | — | — | Single PR, no review burden |
| Phase B (days 1-2) | S12-1 (HotlGateVerdict::Suspend + ticket) + S12-2 (AgentEvent variants + SSE encoder) — one PR, additive types only | S12-3 (DecisionRegistry on AppState) — one PR, no behaviour change | S12-7 (PG store + AppState wiring; flips route from 503 to 201) — one PR, observable behaviour | — | All three parallel; S12-7 has no dep on A/B because the route already exists from sprint-11 |
| Phase C (days 2-4) | — | S12-4 (SuspendingHotlGate adapter; gated by config flag default false → still no behaviour change in prod) — one PR | S12-6 (route handler calls registry.resolve) — one PR, depends on S12-3 + S12-7 | — | S12-4 + S12-6 land together to keep semantics coherent |
| Phase D (days 4-6) | S12-5 (ReAct loop arm; flips behaviour when `agent.hotl.suspend_on_escalate=true`) + S12-9 (3 backend integration tests) — one PR | — | — | S12-8 (frontend SSE + banner state machine) + S12-10 (3 e2e cases) — one PR | Both depend on Phase B/C merging |
| Phase E (day 6) | — | — | — | S12-11 (LLD §4.3.2 post-impl amendment in design repo) | Doc-only PR in `xiaoguai-agent-design` |

PRs in impl repo: 6 (Phase A + 3 in Phase B + 2 in Phase C + 2 in Phase D combined) → 6 + 1 design repo PR = **7 PRs total**. No stacked-PR rebase choreography because Phase B is fully parallel; Phase C waits on Phase B's merge; Phase D waits on Phase C.

Worktree pattern: per [[sprint-workflow]] + sprint-11's success, use one isolated git worktree per stream so the parallel sub-agents can build/test independently. Sub-agent boundaries match phase column boundaries.
...
Step 4 ⟶  7 PRs per §2 sub-agent dispatch (6 impl + 1 design follow-up)
Step 5–7 ⟶ merge dependency order, push, tag v1.9.0
```
...
| # | Check | Result |
|---|---|---|
| 1 | Each task has a clear file path or new-file location | ✅ All 11 tasks |
| 2 | Each task has a verifiable success criterion (specific test or behaviour change) | ✅ S12-1/2/3 unit; S12-4/5 unit + smoke; S12-6/7 integration; S12-8 Vitest; S12-9 backend integration; S12-10 e2e; S12-0 release-workflow side-quest is best-effort |
| 3 | No task introduces an abstraction not required by the task | ✅ `DecisionRegistry` is a thin DashMap wrapper (rejected: per-tenant sharding, queue abstractions); `HotlSuspensionTicket` is a one-method struct (rejected: turning it into a trait); config flag is a single bool (rejected: per-scope override matrix → sprint-13) |
| 4 | Risks identified are real and mitigated, not theatrical | ✅ All 7 in §3 came from explore findings + design review, not speculation |
| 5 | Sprint matches the "carry-forward from sprint-11 §4" framing | ⚠ Soft spot — sprint-11 estimated S11-3a.2 at ~2 days; this plan is ~10. Reason: the design estimate covered only the agent-side gate + AgentEvent. The full closure includes PG store wiring (S12-7), frontend reactor (S12-8), and three e2e cases (S12-10) — all of which were implicit in sprint-11 §4 but not budgeted. Reviewer please confirm scope. |
| 6 | Step-1 LLD callouts and this plan agree | ✅ All file paths + variant names + decision references match `lld-agent.md` §4.5 / `api-contract.md` §2.6.2-3 / `lld-chat-ui.md` §4.3.2 verbatim. S12-11 is the post-impl amendment for §4.3.2's status flip. |
...
3. Observe: chat-ui shows `<HotlBanner>` in pending state; loop is suspended (no tool dispatch in audit log yet).
4. From admin-ui or via `POST /v1/hotl/decisions`, approve.
5. Observe: chat-ui banner clears; loop resumes; tool dispatches; `hotl_resolved(allow)` recorded in SSE; `decisions` row carries `resumed=true`.
6. Repeat with deny → tool synthesises failure, agent observes it in next turn.
7. Repeat without responding for 24h → `hotl_resolved(timeout)` fires; tool synthesises failure.
